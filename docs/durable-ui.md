...
- Toast and its colocated `toast()` controller — a global bounded queue mounted in the persistent layout, updated in place for long-running work, paused on hover, focus, hidden pages, and window blur, manually dismissible, and announced through `aria-live`. Flash-message deduplication is an app adapter using stable ids rather than hidden base behavior.
...
Klean infers the framework and conventional source locations. A copied component or composable carries its own local behavior and direct dependencies. Some patterns still require an honest integration point: one Toast renderer belongs in the persistent application layout, a dialog needs an accessible name and a native trigger relationship, and server-backed URL filters use Inertia. The imported `toast()` function itself needs no provider or setup. Controlled dialog state is optional. Klean documents and generates those source changes directly instead of hiding them in configuration.
