  ...
  function triggers() {
    ...
    return [
      ...list.querySelectorAll(
        "button[data-value], a[href][data-value]",
      ),
    ].filter(
      (element) =>
        element.closest('[data-slot="tabs"]') === rootElement,
    );
  }
  ...
  function sync() {
    ...
    if (currentMode === "navigation") {
      ...
      const currentLink = allTriggers.find(
        (link) => tabValue(link) === value,
      );
      const markedLink = allTriggers.find(
        (link) => link.getAttribute("aria-current") === "page",
      );
      const selected = currentLink ?? (value === undefined ? markedLink : undefined);
