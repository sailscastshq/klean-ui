...
for (const [framework, fixture] of Object.entries(FRAMEWORK_FIXTURES)) {
  test(`installs only the native ${framework} Alert`, () => {
    const root = makeFixture({ framework, tailwindMerge: false });
    const dependencyCalls = [];
    const result = installComponent("alert", {
      cwd: root,
      dependencyInstaller: recordingDependencyInstaller(dependencyCalls),
    });
    const destination = resolve(
      root,
      `assets/js/components/ui/alert/Alert.${fixture.extension}`,
    );
...
for (const [framework, fixture] of Object.entries(FRAMEWORK_FIXTURES)) {
  test(`installs only the native ${framework} Avatar`, () => {
    const root = makeFixture({ framework, tailwindMerge: false });
    const dependencyCalls = [];
    const result = installComponent("avatar", {
      cwd: root,
      dependencyInstaller: recordingDependencyInstaller(dependencyCalls),
    });
    const destination = resolve(
      root,
      `assets/js/components/ui/avatar/Avatar.${fixture.extension}`,
    );

    expect(result.plan.registryItems).toEqual(["avatar"]);
    expect(result.plan.files).toHaveLength(1);
    expect(existsSync(destination)).toBe(true);
    expect(readdirSync(dirname(destination))).toEqual([
...
for (const [framework, fixture] of Object.entries(FRAMEWORK_FIXTURES)) {
  test(`installs only the native ${framework} Card`, () => {
    const root = makeFixture({ framework, tailwindMerge: false });
    const dependencyCalls = [];
    const result = installComponent("card", {
      cwd: root,
      dependencyInstaller: recordingDependencyInstaller(dependencyCalls),
    });
    const destination = resolve(
      root,
      `assets/js/components/ui/card/Card.${fixture.extension}`,
    );

    expect(result.plan.registryItems).toEqual(["card"]);
    expect(result.plan.files).toHaveLength(1);
    expect(existsSync(destination)).toBe(true);
    expect(readdirSync(dirname(destination))).toEqual([
...
for (const [framework, fixture] of Object.entries(FRAMEWORK_FIXTURES)) {
  test(`installs only the native ${framework} Separator`, () => {
    const root = makeFixture({ framework, tailwindMerge: false });
    const dependencyCalls = [];
    const result = installComponent("separator", {
      cwd: root,
      dependencyInstaller: recordingDependencyInstaller(dependencyCalls),
    });
    const destination = resolve(
      root,
      `assets/js/components/ui/separator/Separator.${fixture.extension}`,
    );

    expect(result.plan.registryItems).toEqual(["separator"]);
    expect(result.plan.files).toHaveLength(1);
    expect(existsSync(destination)).toBe(true);
    expect(readdirSync(dirname(destination))).toEqual([
...
for (const [framework, fixture] of Object.entries(FRAMEWORK_FIXTURES)) {
  test(`installs only the native ${framework} Tooltip`, () => {
    const root = makeFixture({ framework, tailwindMerge: false });
    const dependencyCalls = [];
    const result = installComponent("tooltip", {
      cwd: root,
      dependencyInstaller: recordingDependencyInstaller(dependencyCalls),
    });
    const destination = resolve(
      root,
      `assets/js/components/ui/tooltip/Tooltip.${fixture.extension}`,
    );
...
for (const [framework, fixture] of Object.entries(FRAMEWORK_FIXTURES)) {
  test(`installs only the native ${framework} Tabs`, () => {
    const root = makeFixture({ framework, tailwindMerge: false });
    const dependencyCalls = [];
    const result = installComponent("tabs", {
      cwd: root,
      dependencyInstaller: recordingDependencyInstaller(dependencyCalls),
    });
    const destination = resolve(
      root,
      `assets/js/components/ui/tabs/Tabs.${fixture.extension}`,
    );
...
for (const [framework, fixture] of Object.entries(FRAMEWORK_FIXTURES)) {
  test(`installs the native ${framework} Pagination and its Inertia Link`, () => {
    const root = makeFixture({ framework, tailwindMerge: false });
    const dependencyCalls = [];
    const result = installComponent("pagination", {
      cwd: root,
      dependencyInstaller: recordingDependencyInstaller(dependencyCalls),
    });
    const destination = resolve(
      root,
      `assets/js/components/ui/pagination/Pagination.${fixture.extension}`,
    );
    const adapter = {
      vue: "@inertiajs/vue3",
    ...
    expect(result.plan.registryItems).toEqual(["pagination"]);
    expect(result.plan.files).toHaveLength(1);
    expect(existsSync(destination)).toBe(true);
    expect(readdirSync(dirname(destination))).toEqual([
