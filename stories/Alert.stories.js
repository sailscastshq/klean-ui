...
const meta = {
  ...
  argTypes: {
    as: {
      ...
      description:
        "Native container chosen from the surrounding document structure.",
    },
    role: {
      control: "select",
    ...
    template: `
      <section class="klean-story-canvas px-5 py-14 sm:px-8 lg:px-12 lg:py-20" aria-labelledby="alert-semantics-title">
        <header class="max-w-3xl">
          <h1 id="alert-semantics-title" class="text-balance text-4xl font-semibold tracking-tighter sm:text-5xl">Visible does not always mean live.</h1>
...
export const Apps = {
  ...
  render: (args) => ({
    ...
    template: `
      <section class="klean-story-canvas px-5 py-14 sm:px-8 lg:px-12 lg:py-20" aria-labelledby="alert-apps-title">
        <header class="max-w-3xl">
          <h1 id="alert-apps-title" class="text-balance text-4xl font-semibold tracking-tighter sm:text-5xl">One surface. Native content.</h1>
        ...
        <div class="mt-12 grid max-w-6xl gap-8 xl:grid-cols-[0.8fr_1.2fr]">
          <article class="space-y-3">
            <Alert as="section" role="note" aria-labelledby="hagfish-alert-title" class="rounded-none border-2 border-black bg-[#fff3c4] p-6 text-black shadow-[5px_5px_0_0_#000] dark:bg-[#fff3c4] dark:text-black">
              ...
              <h2 id="hagfish-alert-title" class="text-xl font-semibold tracking-tight">This invoice has not been sent</h2>
  ...
  play: async ({ canvasElement, args }) => {
    ...
    await userEvent.click(
      canvas.getByRole("button", { name: "Configure source" }),
    );
